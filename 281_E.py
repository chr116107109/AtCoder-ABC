...
for i in range(K):
    ans += S.find_kth_val(i)

a = [ans]
for i in range(1,N-M+1):
    border = S.find_kth_val(K-1)
    S.delete_val(A[i-1],1)
    S.insert_val(A[i+M-1],1)
    if A[i+M-1] <= border and A[i-1] <= border:
        ans += A[i+M-1]
        ans -= A[i-1]
    elif A[i+M-1] <= border:
        ans += A[i+M-1]
        ans -= S.find_kth_val(K)
    elif A[i-1] <= border:
        ans += S.find_kth_val(K-1)
        ans -= A[i-1]

    now = []
    #for i in range(M):
    #    now.append(S.find_kth_val(i))
    #print(now)
