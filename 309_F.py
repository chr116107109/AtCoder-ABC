...
second = [A[i][1] for i in range(N)]
therd = [A[i][2] for i in range(N)]

second_BIT = OrderBIT(second)
therd_BIT = OrderBIT(therd)
for i in range(N):
    second_BIT.insert_val(second[i],1)
    therd_BIT.insert_val(therd[i],1)

A.sort(key=lambda x:x[0])
ans = 'No'
for i in range(N):
    second_BIT.delete_val(second[i],1)
    therd_BIT.delete_val(therd[i],1)
    if second_BIT.find_kth_val(0) > second[i] and therd_BIT.find_kth_val(0) > therd[i]:
        #print(i)
        ans = 'Yes'
        break
