...
class OrderBIT: 
    ...
    def find_lower(self,x):
        if self.count_lower(x) == 0:
            return -10**9
        return self.find_kth_val(self.count_lower(x)-1)
    
A = [[_,list(map(int,input().split()))] for _ in range(N)]

bit = OrderBIT([a[0] for a in A]+ [0, 10**10]) 
d = {}
d[0] = 0
d[10**10] = 10**10
bit.insert_val(0)
bit.insert_val(10**10)

ans = []
A.sort(key = lambda x:x[1][1])
for i,[a,c] in A:
    #print(ans)
    if ans and ans[-1][0] > a:
        continue
...
print(len(ans))
print(*sorted([i for _,_,i in ans]))